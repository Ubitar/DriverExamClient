      ...
      <div-flex-column class="container">
        ...
        <div-flex-row class="header">
          <div-flex-row class="tip">
            ...
            得分：{{score}}分&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <el-button type="danger" size="mini" @click="clickToBeginExam" v-if="timeInterval">
              {{parseInt(time/60)}}:{{parseInt(time%60)}} 提交
            </el-button>
            <el-button type="danger" size="mini" @click="clickToBeginExam" v-else>开始考试</el-button>
          </div-flex-row>
        </div-flex-row>
        <div class="card main">
  ...
  export default {
    ...
    methods: {
      clickToRandomExercise: function () {
        if (this.time < 45 * 60 - 1) {
          this.$message.error('正在模拟开始');
        } else {
      ...
      finish: function (params) {
        this.$Api.stuFinishSimulateExam(this, params).then((res) => {
          if (res.code == 200) {
            this.$message({
              message: "提交成功",
              type: "success"
            })
            this.$router.replace("/client/index/personmsg");
          } else {
            this.$message({
              message: res.msg,
              type: 'error'
