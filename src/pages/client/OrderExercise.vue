    ...
    <div-flex-column class="page">
      <div-flex-column class="container">
        <div-flex-row class="exercise-selectors">
          ...
          <div-flex-row class="button" @click="clickToRandomExercise">
            <div>随机练习</div>
        ...
        <div-flex-row class="header">
          <div-flex-row class="tip">
            ...
            跳转到第：
            <el-input class="input-to" v-model="inputTo" size="mini" maxlength="4"></el-input>&nbsp;题&nbsp;
            <el-button type="primary" size="mini" @click="clickSkipTo">跳 转</el-button>&nbsp;&nbsp;&nbsp;&nbsp;
...
<script>
  ...
  export default {
    ...
    methods: {
      clickToRandomExercise: function () {
        this.$router.replace("/client/index/randomexercise?level=" + this.level + "&type=" + this.type);
      },
      ...
      clickSkipTo: function () {
        ...
        if (this.inputTo > this.questionIds.length) this.index = this.questionIds.length - 1;
        else if (this.inputTo < 0) this.index = 0;
        else this.index = this.inputTo - 1;
        this.loadTo({
          type: this.type,
          level: this.level,
          id: this.questionIds[this.index]
      ...
      loadLastId: function (params) {
        this.$Api.stuGetOrderExerciseCurIndex(this, params).then((res) => {
          if (res.code == 200) {
            var data = res.data;
            switch (this.type) {
              case "A1_A3_B1":
                if (this.level == 1) this.lastQusetionId = data.a1_A3_B1_1_id;
                else if (this.level == 4) this.lastQusetionId = data.a1_A3_B1_4_id;
                break;
              case "B2_A2":
                if (this.level == 1) this.lastQusetionId = data.b2_A2_1_id;
                else if (this.level == 4) this.lastQusetionId = data.b2_A2_4_id;
                break;
              case "C1_C2_C3":
                if (this.level == 1) this.lastQusetionId = data.c1_C2_C3_1_id;
                else if (this.level == 4) this.lastQusetionId = data.c1_C2_C3_4_id;
                break;
              case "E_F_D":
                if (this.level == 1) this.lastQusetionId = data.e_F_D_1_id;
                else if (this.level == 4) this.lastQusetionId = data.e_F_D_4_id;
                break;
              default:
                return;
            }
            ...
            this.index = this.questionIds.indexOf(this.lastQusetionId);
            if (this.index < 0) {
              this.index = 0;
              params.id = this.questionIds[this.index];
              this.lastQusetionId = params.id
            }
            this.loadTo(params)
          } else {
            this.$message({
      ...
      finish: function (params) {
        this.$Api.stuFinishOrderExercise(this, params).then((res) => {
          if (res.code == 200) {
          } else {
            this.$message({
              message: res.msg,
